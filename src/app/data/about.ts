...
실무에서 다루지 못한 React·TypeScript 기반 SPA 설계와 서버 상태 관리는 개인 프로젝트로 채웠습니다. 남에게 보여줄 예제가 아니라 제가 직접 쓸 도구를 만들고, 실제로 사용하면서 발견한 문제를 고쳐 나가는 방식으로 진행했습니다. 북마크 앱은 지금도 매일 쓰고 있고, 쓰다가 발견한 분류 오류나 불필요한 네트워크 호출을 그때그때 개선했습니다.`,
};
  ...
  projects: [
    {
      ...
      duties: [
        "교육부 가이드라인을 분석해 공공 플랫폼 환경에 맞는 UI/UX로 구체화",
        "교과서 분량의 학습 콘텐츠 대량 개발(양산)",
        "공공 플랫폼 납품 기준에 맞춘 표준 마크업/스타일 적용",
        "배포 이후 발생한 운영 이슈 대응·기능 개선을 상시 담당하며 서비스 안정성 유지",
        "번들링 최적화를 병행해 콘텐츠 로딩 구조 개선",
        "기획팀·영상팀 등 유관 부서와 협업하며 요구사항을 기술 관점으로 구체화하고 구현 가능 범위를 조율",
      ],
    },
    {
      ...
      duties: [
        "시맨틱 마크업 기반으로 학습 콘텐츠를 구현하고, 키보드 내비게이션·스크린 리더 동작을 직접 검증",
        "드래그·클릭 등 마우스 전용 인터랙션에 키보드 대안 인터랙션을 별도 설계해 조작 수단과 관계없이 동일한 학습 흐름 제공",
        "교과서 단원별 인터랙티브 퀴즈·학습 활동 구현",
        "크로스 브라우저 및 반응형 대응",
        "학습 이력 저장 로직과 연동한 평가 모듈 구현 (AI 학습 분석 데이터로 활용)",
      ],
    },
  ],
  extraLink: {
